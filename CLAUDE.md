...
- **コメントベースヘルプ**: 関数には PowerShell のコメントベースヘルプ (`.SYNOPSIS` 等) を日本語で記載・更新する
- **PowerShell 互換性**: `bin/` のスクリプトは Windows PowerShell 5.1 以降と PowerShell Core の両方で動作させる (CI が両方でテストするため)
